...
//5. Play Functions - START

void printframe(point_t curpoint, point_t endpoint, char grid[256][256], int gridwidth,
		int gridlines, int moves)
{
	int ix,iy;
	...
	for (iy=0; iy<gridlines; iy++)
	{
		...
		for (ix=0; ix<gridwidth; ix++)
		{
			if ((curpoint.x == ix) && (curpoint.y == iy))
				printf("P");
			else if ((endpoint.x == ix) && (endpoint.y == iy))
				printf("X");
			else
...
int dirslide(point_t *curpos, point_t endpoint, int dir, char grid[256][256], int gridwidth,
		int gridlines)
{
	point_t slidedest = dirmove(*curpos, endpoint, dir, grid, gridwidth, gridlines);

	if (slidedest.x != -1) // -1 means did not move
	{ // not -1
		curpos->x = slidedest.x;
		curpos->y = slidedest.y;
		return 1; //moved
	}
	else // -1
		return 0; //did not move
}


//5. Play Functions - END
...
void main()
{
	...
	point_t currentpos = startpoint;
	int moves = 0;
	int solved = 0;

	printframe(currentpos, endpoint, grid, gridwidth, gridlines, moves);

	if (ptsequal(currentpos,endpoint)) solved = 1;

	while(!solved)
	{ // OPTION TO REPLAY PUZZLE - START
	while(!solved) // Haven't reached finish
	{ // LOOP: INPUT AND PUZZLE SOLVING - START
		char cinput[51] = "";
		...
		if (cinput[0] == 'a') dir = 2;
		if (cinput[0] == 'd') dir = 3;
		if (cinput[0] == 'r') dir = 10; // r - reset
		if (cinput[0] == 'q') dir = 11; // q = quit

		if ((dir >= 0) && (dir <= 3))
		{
			if(dirslide(&currentpos, endpoint, dir, grid, gridwidth, gridlines))
				moves++;
		}
		...
		if (ptsequal(currentpos,endpoint))
		{
			printf("\n");
			solved = 1;
		}

	} // LOOP: INPUT AND PUZZLE SOLVING - END
	...
	if (solved == 1)
	{
		...
		if (moves <= minstepsreq)
			printf(" YOU HAVE SOLVED THE PUZZLE IN THE MINIMUM AMOUNT OF STEPS!\n");
		...
		while(!questionanswered)
		{
			...
			if ((cinput[0] == 'n') || (cinput[0] == 'N'))
			{
				questionanswered = 1; // QUIT PUZZLE
			}
			else if ((cinput[0] == 'y') || (cinput[0] == 'Y'))
			{
				questionanswered = 1;
				solved = 0; // go back to solving.
	...
	} // OPTION TO REPLAY PUZZLE - END

	}// ABLE TO READ FILE - END

	printf("\n\n\n");
	} // WHILE: LOOP WHOLE PROGRAM - END
}
